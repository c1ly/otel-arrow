...
- [x] Generation/ownership guards prevent stale acks from committing revoked partitions (`receiver.rs:596` stale-generation guard, `receiver.rs:609` late-ack guard, `acks_for_revoked_partition` counter).
  - The classifier is exhaustively unit-tested (`classify_offset_feedback_*`, seven cases) and the reassign-before-retrack window by `stale_same_gen_ack_dropped_after_reassignment_before_retrack`. End-to-end proof of the counter path for both feedback kinds: `stale_ack_after_revoke_counts_acks_for_revoked_partition` (Ack) and `stale_nack_after_revoke_counts_acks_for_revoked_partition` (terminal permanent Nack) in `receiver.rs` -- the receiver holds an un-acked in-flight record, a `RebalanceTrigger` steals the partition, and the now-stale Ack/Nack increments the terminal `acks_for_revoked_partition` (the stale feedback is dropped, never committed to the revoked partition; Ack and Nack share `handle_offset_feedback`). The receiver-state contract that an old-generation Ack/Nack does not move the commit offset is asserted directly by `stale_generation_ack_does_not_advance_committable_offset` (after revoke+reassign under a newer generation, the stale gen-1 feedback classifies `DropStale`, acking its old offset is a no-op, and the committable offset reflects only the new generation) and by the tracker-level `stale_generation_records_not_committed_after_reassignment` / `revoke_reassign_commits_only_new_generation`.
  - Idempotency + generation interaction: the idempotency dedupe on the receive path is generation-aware (`is_known_offset_for_generation`). A message redelivered under a NEWER ownership generation (same offset, after a revoke+reassign) is never treated as a known duplicate -- it is reprocessed, and tracking it resets the partition's stale old-generation state to the new generation. Dedupe therefore applies only WITHIN an ownership generation, never across one, so a record a new owner is responsible for is never wrongly suppressed. Proven by the unit test `is_known_offset_for_generation_allows_newer_generation_same_offset` (newer generation => not known; same generation => still deduped) and the end-to-end `idempotent_redelivery_under_new_generation_is_reprocessed_not_skipped` (a `RebalanceTrigger` join/leave forces redelivery under a new generation; the redelivered records are reprocessed and `idempotent_skips` stays 0). Complementary tracker-state clearing is proven by `is_known_offset_false_for_old_offset_after_newer_generation_track` (in-place reset path) and `is_known_offset_false_for_old_offset_after_revoke_reassign` (purge path) in `offset_tracker.rs`.
- [x] In-flight records crossing an assignment change.
  - Bounded-duplication / no-loss / no-double-commit across a rebalance is covered by `rebalance_two_receivers_scale_up_down_distribute_without_loss_or_double_commit` (committed-offset equality + broker retention). Cross-generation redelivery under idempotency mode is covered by `idempotent_redelivery_under_new_generation_is_reprocessed_not_skipped` in `receiver.rs`: an idempotency-enabled receiver holds uncommitted offsets across a `RebalanceTrigger` join/leave that reassigns the partitions under a new generation, and the redelivered records are reprocessed (delivered again) rather than skipped, since idempotent dedupe applies only within an ownership generation.
